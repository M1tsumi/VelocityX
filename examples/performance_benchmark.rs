...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    run_throughput_benchmark(
        ...
        4, 4, 250_000, 1_000_000
    )?;
    
    run_throughput_benchmark(
        "8:8 (8 producers, 8 consumers)",
        8, 8, 125_000, 1_000_000
    )?;
...
fn run_latency_benchmark() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("    Measuring end-to-end latency with {} samples...", num_samples);
...
fn run_contention_benchmark() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("    High contention test: {} threads, {} ops each", 
...
fn run_memory_benchmark() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for capacity in capacities {
        ...
        println!("    Capacity {}: {} bytes ({:.2} MB) buffer, {:.2} MB with padding",
                 capacity,
                 memory_usage,
...
fn run_error_handling_benchmark() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("    Testing error handling performance with {} error operations...", num_error_ops);
    ...
    println!("      Average error latency: {:.2} ns/error", 
